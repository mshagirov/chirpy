...
const setChirpyRedWithID = `-- name: SetChirpyRedWithID :one
...
type SetChirpyRedWithIDParams struct {
	ID          uuid.UUID
	IsChirpyRed bool
}

func (q *Queries) SetChirpyRedWithID(ctx context.Context, arg SetChirpyRedWithIDParams) (User, error) {
	row := q.db.QueryRowContext(ctx, setChirpyRedWithID, arg.ID, arg.IsChirpyRed)
	var i User
	err := row.Scan(
		&i.ID,
		&i.CreatedAt,
...
func (q *Queries) UpdateUserWithID(ctx context.Context, arg UpdateUserWithIDParams) (User, error) {
	row := q.db.QueryRowContext(ctx, updateUserWithID, arg.ID, arg.Email, arg.HashedPassword)
	var i User
	err := row.Scan(
		&i.ID,
		&i.CreatedAt,
